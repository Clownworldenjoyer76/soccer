...
# filters from markets.yaml. Normal selection files remain rebuildable.
#
# In addition, the pipeline run date is written once to:
#   docs/win/soccer/04_select/locked/{date}_soccer_bets.csv
# Locked files are never deleted or overwritten by this script and contain the
# SHA-256 of markets.yaml that produced the selections.
...
def load_config() -> tuple[dict, dict[str, str]]:
    try:
        ...
        missing = [
            key for key in MATCH_ODDS_SOURCE_KEYS if key not in source_config
        ]
        if missing:
            raise ValueError(
        ...
        _log(
            f"CONFIG LOAD FAILED | {CONFIG_PATH} | {e}\n{traceback.format_exc()}",
            "ERROR",
        )
        raise
...
def market_cfg(league, market_type):
    try:
        ...
        raise KeyError(
            f"No config: league={league!r} market_type={market_type!r}"
        ) from e
...
def validate_match_odds_provenance(row, side: str) -> None:
    ...
    for metric, source_field in checks.items():
        expected = probability_column(
            side, MATCH_ODDS_PROBABILITY_SOURCES[metric]
        )
        actual = row.get(source_field)
        if actual is None or pd.isna(actual):
            raise ValueError(
                f"Missing 1X2 provenance field {source_field!r}; "
                "rerun build_edges.py before selection"
            )
        actual = str(actual).strip()
        if actual != expected:
            raise ValueError(
                f"Stale/inconsistent 1X2 provenance for side={side} "
                f"metric={metric}: markets.yaml expects {expected!r}, "
                f"stage-3 row says {actual!r}. Rerun build_edges.py before selection."
            )
...
def lock_daily_picks(
    ...
    if lock_date not in seen_input_dates:
        _log(
            f"LOCK SKIPPED | no stage-3 input files found for run date {lock_date}",
            "WARN",
        )
    ...
    _log(
        f"LOCK CREATED | {locked_path} | rows={len(locked)} | "
            ...
            sides.append(
                make_side(
                    ...
                    model_prob_source=str(
                        row.get(f"{side}_selection_prob_source")
                    ).strip(),
...
def process_file(file: Path):
    ...
    if league_key not in CONFIG:
        _log(
            f"SKIP league not in config: {file.name} (league={league_key!r})",
            "WARN",
        )
        return [], "skip"
    ...
    preference = cfg.get(
        "pick_preference", {"metric": "ev", "direction": "max"}
    )
