...
def discover_merge_dates(merge_dir: Path, cutoff_date: str) -> list[str]:
    ...
    if not merge_dir.exists():
        return []
    for path in merge_dir.glob("*_ligue1_*.csv"):
...
def load_bundle(joblib, root: Path, item):
    ...
    if not path.exists():
        raise FileNotFoundError(f"Required Ligue 1 model missing: {path}")
...
def consolidate_duplicate_games(current: pd.DataFrame) -> pd.DataFrame:
    ...
    if current["game_id"].isna().any():
        raise RuntimeError("Ligue 1 inference stopped: blank or invalid game_id.")
    ...
    for game_id, group in current.groupby("game_id", sort=False, dropna=False):
        ...
        message = (
            f"Ligue 1 ML inference: consolidated {len(group)} sportsbook rows "
    ...
    if consolidated["game_id"].duplicated().any():
        raise RuntimeError(
            "Ligue 1 inference stopped: duplicate game_id remained after consolidation."
        )

    print(
        "Ligue 1 ML inference: sportsbook duplicate consolidation complete: "
        f"{duplicate_game_count} game(s), {removed_row_count} duplicate row(s) removed."
    )
    return consolidated


def make_feature_frame(ligue1: pd.DataFrame) -> pd.DataFrame:
    dates = pd.to_datetime(
        ligue1["match_date"].astype("string").str.strip().str.replace("_", "-", regex=False),
        errors="coerce",
    )
    if dates.isna().any():
        bad = ligue1.loc[
        ...
        raise RuntimeError(
            "Ligue 1 inference stopped: unparseable match_date rows:\n"
...
def validate_predictions(predicted: pd.DataFrame):
    ...
    if missing:
        raise RuntimeError(
            f"Ligue 1 inference stopped: required model outputs absent: {missing}"
        )
    ...
    for column in PROBABILITY_COLUMNS:
        ...
        if (
            ...
            raise RuntimeError(
                f"Ligue 1 inference stopped: invalid probability output in {column}."
            )
    ...
    for column in GOAL_COLUMNS:
        ...
        if not np.isfinite(values).all() or (values < 0.0).any():
            raise RuntimeError(
                f"Ligue 1 inference stopped: invalid goal output in {column}."
            )
    ...
    if not np.allclose(
        ...
        raise RuntimeError(
            "Ligue 1 inference stopped: 1X2 probabilities do not sum to 1."
        )
    ...
    for a, b in (
        ...
        if not np.allclose(
            ...
            raise RuntimeError(
                f"Ligue 1 inference stopped: {a} + {b} does not equal 1."
            )
...
def enrich_merge_file(path: Path, predictions: pd.DataFrame) -> int:
    ...
    if "game_id" not in df.columns:
        raise RuntimeError(
            f"Ligue 1 inference stopped: game_id absent from merged file {path}"
        )

    df["game_id"] = df["game_id"].map(normalize_game_id)
    if df["game_id"].isna().any():
        raise RuntimeError(
            f"Ligue 1 inference stopped: blank game_id in merged file {path}"
        )
    if df["game_id"].duplicated().any():
        dupes = df.loc[df["game_id"].duplicated(keep=False), "game_id"].tolist()
        raise RuntimeError(
            f"Ligue 1 inference stopped: duplicate game_id in merged file {path}: {dupes}"
        )
    ...
    if missing_predictions.any():
        ...
        raise RuntimeError(
            "Ligue 1 inference stopped: merged rows have no model prediction "
            f"in {path}:\n" + bad.to_string(index=False)
        )
...
def process_date(date_text: str, soccer_root: Path, bundles):
    ...
    merge_paths = [
        merge_dir / f"{date_text}_ligue1_{suffix}.csv"
    ...
    sportsbook_path = (
        ...
        / f"{date_text}_ligue1.csv"
    )
    if not sportsbook_path.exists():
        raise FileNotFoundError(
            "Ligue 1 inference stopped: merge files exist for "
    ...
    if current.empty:
        raise RuntimeError(
            "Ligue 1 inference stopped: normalized sportsbook file is empty "
    ...
    print(
        "Ligue 1 ML inference complete for "
...
def main():
    ...
    bundles = load_all_bundles(joblib, ligue1_root)
